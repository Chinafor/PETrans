...
if __name__ == '__main__':
    ...
    parser.add_argument('--run_name', type=str,
                        help="name for wandb run", required=False,default='test')
    parser.add_argument('--debug', action='store_true',
                        default=False, help='debug')
    # in moses dataset, on average, there are only 5 molecules per scaffold
    parser.add_argument('--scaffold', action='store_true',
                        default=True, help='condition on scaffold')
    parser.add_argument('--lstm', action='store_true',
                        default=False, help='use lstm for transforming scaffold')
    parser.add_argument('--data_name', type=str, default='egfr_moses73',
    ...
    #print(train_dataset.vocab_size)#94
    # print(train_dataset.max_len)78
    # print('*******')
    mconf = GPTConfig(train_dataset.vocab_size, 78, num_props=num_props,  # args.num_props,
    ...
    #args.vocab_size=94---100
    #args.block_size=54---78
    # mconf = GPTConfig(100, 78, num_props = num_props,
    #                    n_layer=args.n_layer, n_head=args.n_head, n_embd=args.n_embd, scaffold = args.scaffold, scaffold_maxlen = scaffold_max_len,
    #                    lstm = args.lstm, lstm_layers = args.lstm_layers)
    model = GPT(mconf)
    model = torch.load('1.pt')
    #model.load_state_dict(torch.load('../cond_gpt/weights/scaffold_moses_2rgp_617.pt'))
    #model.to('cuda')
    #print('Model loaded')
...
# #冻结
# # for param in model.parameters():  # 遍历每个权重参数model.x.parameters
# #     param.requires_grad = False  # 保留梯度为False
# for param in model.tok_emb.parameters():  # 遍历每个权重参数model.x.parameters
#     param.requires_grad = False  # 保留梯度为False
# for param in model.type_emb.parameters():  # 遍历每个权重参数model.x.parameters
#     param.requires_grad = False  # 保留梯度为False
# for param in model.drop.parameters():  # 遍历每个权重参数model.x.parameters
#     param.requires_grad = False  # 保留梯度为False
# for param in model.blocks.parameters():  # 遍历每个权重参数model.x.parameters
#     param.requires_grad = False  # 保留梯度为False
# #ln_f,head
# model.to('cuda')
