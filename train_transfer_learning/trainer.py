...
class Trainer:
    ...
    def save_checkpoint(self):
        ...
        #torch.save(raw_model,'new.pt')
        torch.save(raw_model,'test.pt')
    ...
    def train(self, wandb):
        ...
        def run_epoch(split):
            ...
            for it, (x, y, p, scaffold) in pbar:
                ...
                if is_train:
                    #add
                    #model.load_state_dict(torch.load('../cond_gpt/weights/scaffold_moses_2rgp.pt'),strict=False)
                    # backprop and update the parameters
                    model.zero_grad()#梯度归零
                    scaler.scale(loss).backward()#反向传播
                    scaler.unscale_(optimizer)#对optimizer指定的参数的梯度进行反缩放操作
                    torch.nn.utils.clip_grad_norm_(model.parameters(), config.grad_norm_clip)#梯度裁剪
                    scaler.step(optimizer)#如果出现了inf或者NaN，scaler.step(optimizer)会忽略此次的权重更新
                    scaler.update()#更新
